...
export const validateCombinationIngredients = (
  ...
  const errors = day.items
    ...
    .flatMap((item) => {
      const missingIngredients = (item.ingredientFoodIds ?? []).filter((foodId) => {
      ...
      return [
        `Cannot add combination ${item.label}. The following ingredients have not been introduced beforehand: ${missingIngredients.join(", ")}.`,
      ];
    });
